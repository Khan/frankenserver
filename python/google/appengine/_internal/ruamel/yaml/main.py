...
from google.appengine._internal.ruamel.yaml.compat import StringIO, BytesIO, with_metaclass, PY3
from google.appengine._internal.ruamel.yaml.resolver import VersionedResolver, Resolver  # NOQA
from google.appengine._internal.ruamel.yaml.representer import (BaseRepresenter, SafeRepresenter, Representer,
                                     RoundTripRepresenter)
from google.appengine._internal.ruamel.yaml.constructor import (BaseConstructor, SafeConstructor, Constructor,
...
class YAML(object):
    def __init__(self, _kw=enforce, typ=None, pure=False, plug_ins=None):
        ...
        if self.typ == 'rt':
            ...
            self.Emitter = google.appengine._internal.ruamel.yaml.emitter.Emitter if pure or CEmitter is None else CEmitter
    ...
    def get_serializer_representer_emitter(self, stream, tlca):
        ...
        rslvr = google.appengine._internal.ruamel.yaml.resolver.BaseResolver if self.typ == 'base' else google.appengine._internal.ruamel.yaml.resolver.Resolver
...
def add_implicit_resolver(tag, regexp, first=None, Loader=None, Dumper=None,
    ...
    if Loader:
        ...
        elif issubclass(Loader, (BaseLoader, SafeLoader, google.appengine._internal.ruamel.yaml.loader.Loader,
                                 RoundTripLoader)):
            Resolver.add_implicit_resolver(tag, regexp, first)
        else:
            raise NotImplementedError
    if Dumper:
        ...
        elif issubclass(Dumper, (BaseDumper, SafeDumper, google.appengine._internal.ruamel.yaml.dumper.Dumper,
                                 RoundTripDumper)):
            Resolver.add_implicit_resolver(tag, regexp, first)
        else:
            raise NotImplementedError
